...
void TriangulatorWorker::triangulatePointCloud(cv::Mat up, cv::Mat vp,
  ...
  busy = false;
  if (!result) {
    std::cerr << "TriangulatorWorker: dropped sequence." << std::endl;
    return;
  }
  ...
  emit logMessage(QString("Triangulator: %1ms").arg(timer.elapsed()));
