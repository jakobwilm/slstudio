...
CalibratorRBF::CalibratorRBF(QObject *parent, unsigned int _screenCols,
                             unsigned int _screenRows)
    : Calibrator(parent, _screenCols, _screenRows) {

  // Create encoder/decoder
  encoder = std::make_unique<EncoderCalibration>(screenCols, screenRows,
                                                 CodecDirBoth);
  decoder = std::make_unique<DecoderCalibration>(screenCols, screenRows,
                                                 CodecDirBoth);
...
CalibrationData CalibratorRBF::calibrate() {
  ...
  //        qc_proj[i]); float err = cv::norm(cv::Mat(qc[i]),
  //        cv::Mat(qc_proj[i]), cv::L2); cam_error_per_view[i] =
  //        (float)std::sqrt(err*err/n);

  //        cv::projectPoints(cv::Mat(Q[i]), proj_rvecs[i], proj_tvecs[i], Kp,
  //        kp, qp_proj[i]); err = cv::norm(cv::Mat(qp[i]), cv::Mat(qp_proj[i]),
  //        cv::L2); proj_error_per_view[i] = (float)std::sqrt(err*err/n);
  ...
  //        fs << QString("cam_rvecs_%1").arg(i).toStdString() <<
  //        cv::Mat(cam_rvecs[i]); fs <<
  //        QString("cam_tvecs_%1").arg(i).toStdString() <<
  //        cv::Mat(cam_tvecs[i]); fs <<
  //        QString("proj_rvecs_%1").arg(i).toStdString() <<
  //        cv::Mat(proj_rvecs[i]); fs <<
  //        QString("proj_tvecs_%1").arg(i).toStdString() <<
  //        cv::Mat(proj_tvecs[i]); fs <<
  //        QString("qc_proj_%1").arg(i).toStdString() << cv::Mat(qc_proj[i]);
  //        fs << QString("qp_proj_%1").arg(i).toStdString() <<
  //        cv::Mat(qp_proj[i]); fs <<
  //        QString("cam_error_per_view_%1").arg(i).toStdString() <<
  //        cam_error_per_view[i]; fs <<
  //        QString("proj_error_per_view_%1").arg(i).toStdString() <<
