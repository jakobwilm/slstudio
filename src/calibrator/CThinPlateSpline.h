...
This is the header file of the CThinPlateSpline class. All functionality is
wraped and packed in this file. This class is used to compute non-rigid
deformations using the algorithm introduced by Bookstein in the late 80's. The
original paper "Principal Warps: Thin-Plate Splines and the Decomposition of
Deformations" can be found here
http://cseweb.ucsd.edu/classes/sp03/cse252/bookstein.pdf

The alorithm was somehat improved but is nearly identical with the original
proposed in the original paper. There are plans to implement also some
approximations and more runtime efficient versions which were described here
http://cseweb.ucsd.edu/~sjb/pami_tps.pdf.

Up until now there is only the possibility to use the spline algorithm to warp
either forward or backward according to the energy evaluation of the
corresponding points given by the user.
...
public:
  ...
  **************************************************************************
  ...
  @param pS                       Reference to a STL vector that holds the
  points of the source image.

  @param pD                       Reference to a STL vector that holds the
  points of the target image.
  ***************************************************************************/
  CThinPlateSpline(const std::vector<Point2f> &pS,
  ...
  **************************************************************************
  ...
  As one can easily see the computation of one transformed pixel takes
/f$O(n^2)/f$ operations where n is the number of corresponding points.
  ...
  @return                                 The function returns the interpolation
result as cv::Point2f-structure
  ***************************************************************************/
  ...
  **************************************************************************
  ...
  Nothing will be done with this array other than reading values.

  @param dst                                      Reference the transformation
  destination. If dst was allocated before, it will be released and initialized
  with the correct size, depth etc.

  @param interpolation            Reference the transformation destination. If
  dst was allocated before, it will be released and initialized with the correct
  size, depth etc. All known interpolation function from the OpenCV library are
  possible: <br> INTER_NEAREST, <br> INTER_LINEAR, <br> INTER_AREA, <br>
  INTER_CUBIC, <br>
  INTER_LANCZOS4

  @param tpsInter                         This parameter defines the direction
  of the interpolation function. Possible values are: <br> FORWARD_WARP, <br>
  BACK_WARP
  ...
  **************************************************************************
  ...
  @return                                 void
  ***************************************************************************/
  void setCorrespondences(const std::vector<Point2f> &pS,
                          const std::vector<Point2f> &pD);
  ...
  **************************************************************************
  ...
  @param dstSize                  size of the destination image. Here you can
  insert any size you like. The algorithm will simply evaluate the spline and
  look on the correct position in the array. If there is no value, the result
  will be black.
...
private:
  ...
  ****************************************************************************
  ...
  @return                 the function will return a double value computed with
  the above mentioned function.

  ****************************************************************************/
  double fktU(const Point2f &p1, const Point2f &p2);

  void computeSplineCoeffs(std::vector<Point2f> &iP, std::vector<Point2f> &iiP,
                           float lambda,
                           const TPS_INTERPOLATION tpsInter = BACK_WARP);
