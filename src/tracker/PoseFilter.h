...
signals:
  ...
  // void imshow(const char* windowName, cv::Mat im, unsigned int x, unsigned
  // int y);
private:
  QTime time;

  // Extended Kalman filter structures
  // Notation according to Goddard "Pose and Motion Estimation...", U Tennessee,
  // 1997
  Eigen::Matrix<float, 13, 1>
      s; // state [tx,ty,tz,qw,qx,qy,qz,vx,xy,vz,ox,oy,oz]
