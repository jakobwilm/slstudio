...
OpenGLContext::OpenGLContext(uint _screenNum) : screenNum(_screenNum) {
  ...
  XVisualInfo *visualInfo =
      glXChooseVisual(contextInfo->display, screenNum, attrListDbl);
  ...
  // Create colormap
  Colormap colormap = XCreateColormap(contextInfo->display, contextInfo->window,
  ...
  contextInfo->window = XCreateWindow(
      contextInfo->display, contextInfo->window, 0, 0, screenResX, screenResY,
      0, visualInfo->depth, InputOutput, visualInfo->visual, wamask, &wa);
  ...
  XChangeWindowAttributes(contextInfo->display, contextInfo->window,
  ...
  glXMakeCurrent(contextInfo->display, contextInfo->window,
                 contextInfo->context);
  ...
  const char *glx_extensions =
      glXQueryExtensionsString(contextInfo->display, screenNum);
  if (strstr(glx_extensions, "GLX_SGI_swap_control")) {
    ...
    std::cerr
        << "OpenGLContext.Unix Error: Could not access swap interval extension!"
        << std::endl;
  }
...
OpenGLContext::~OpenGLContext() {
  ...
  if (contextInfo->context) {
    ...
    if (!glXMakeCurrent(contextInfo->display, None, NULL))
      std::cerr << "Error. Could not release drawing OpenGLContext."
                << std::endl;
